...
digCharacteristic::digCharacteristic(char* bleuuid) : 
     BLECharacteristic(bleuuid, BLERead | BLEWrite | BLENotify, LenDigBits) 
{ 
...
{
	word changed = pinIO::produceBLEdig(digdata, LenDigBits);  // get digdata
	return changed;
}
...
digCharacteristic digc (UUID16_CHR_DIGITAL); // global digital chracterist

void setupDIGC(BLEService& aios)
{
	pinIO::createDigBits(nDigBits);
	aios.addCharacteristic(digc);
	Serial.print("setupDIGC: nDigBits="); Serial.println(nDigBits);
}
